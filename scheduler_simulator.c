...
#define DURATION 5  // Define a macro for the default duration of each job, set to 5 seconds
...
{ // Loop through each job
    ...
    {                                                                                     // Print message indicating the start of a job's execution
        printf("Executing Job %d for %d seconds...\n", jobs[i].job_id, jobs[i].duration); // Pause the program execution for the duration of the current job
        sleep(jobs[i].duration);                                                          // Print message indicating the completion of a job's execution
